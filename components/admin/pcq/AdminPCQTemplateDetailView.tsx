...
import {
  ADMIN_ADD_PCQ_QUESTION_MUTATION,
  ADMIN_CREATE_PCQ_TEMPLATE_VERSION_MUTATION,
  ADMIN_DELETE_PCQ_QUESTION_MUTATION,
  ADMIN_DELETE_PCQ_TEMPLATE_MUTATION,
  ADMIN_PCQ_TEMPLATE_DETAIL_QUERY,
  ADMIN_PCQ_TEMPLATES_QUERY,
  ADMIN_PUBLISH_PCQ_TEMPLATE_VERSION_MUTATION,
  ADMIN_UPDATE_PCQ_QUESTION_MUTATION,
  ADMIN_UPDATE_PCQ_TEMPLATE_MUTATION,
...
function EditTemplateForm({
  ...
  const [updateTemplate, { loading }] = useMutation(ADMIN_UPDATE_PCQ_TEMPLATE_MUTATION);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      await updateTemplate({
        ...
        refetchQueries: [
          { query: ADMIN_PCQ_TEMPLATE_DETAIL_QUERY, variables: { id: template.id } },
          ADMIN_PCQ_TEMPLATES_QUERY,
        ],
      });
...
// ─── Question Card (view + inline edit + delete) ───────────────────────────────

function QuestionCard({
  question,
  templateId,
  onDone,
}: Readonly<{ question: PCQQuestion; templateId: string; onDone: (alert: AlertState) => void }>) {
  ...
  const [updateQuestion, { loading: updating }] = useMutation(ADMIN_UPDATE_PCQ_QUESTION_MUTATION);
  const [deleteQuestion, { loading: deleting }] = useMutation(ADMIN_DELETE_PCQ_QUESTION_MUTATION);

  const options = parseOptions(question.options);
  const refetch = [{ query: ADMIN_PCQ_TEMPLATE_DETAIL_QUERY, variables: { id: templateId } }];
...
function AddQuestionForm({
  ...
  const [addQuestion, { loading }] = useMutation(ADMIN_ADD_PCQ_QUESTION_MUTATION);
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      await addQuestion({
        ...
        refetchQueries: [{ query: ADMIN_PCQ_TEMPLATE_DETAIL_QUERY, variables: { id: templateId } }],
      });
...
export function AdminPCQTemplateDetailView({ templateId }: Readonly<{ templateId: string }>) {
  ...
  const [createVersion, { loading: creatingVersion }] = useMutation(ADMIN_CREATE_PCQ_TEMPLATE_VERSION_MUTATION);
  const [publishVersion, { loading: publishing }] = useMutation(ADMIN_PUBLISH_PCQ_TEMPLATE_VERSION_MUTATION);
  const [deleteTemplate, { loading: deletingTemplate }] = useMutation(ADMIN_DELETE_PCQ_TEMPLATE_MUTATION);
  ...
  const refetchDetail = [{ query: ADMIN_PCQ_TEMPLATE_DETAIL_QUERY, variables: { id: templateId } }];
      ...
      {editingTemplate ? (
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {template.specialtyId ? (
            <span className="text-xs text-muted">Specialty: {template.specialtyId}</span>
          ) : null}
        </div>
      )}
          ...
          <CardHeader>
            ...
            {displayVersion.isCurrent ? (
              <p className="text-xs text-muted">
                This is the active version. Editing questions here affects new appointments.
              </p>
            ) : null}
          </CardHeader>
